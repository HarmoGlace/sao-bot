...
class Handler extends AkairoHandler {
    ...
    runLocationCheck(msg, command) {
        ...
        if (!msg.channel.guild && !locations.includes("dm")) {
            this.emit("locationError", msg, command, locations);
            return true;
        }

        if (msg.channel.guild && !locations.includes("guild")) {
            this.emit("locationError", msg, command, locations);
            return true;
        }
    ...
    runCompetitionCheck (msg, command) {
        ...
        if (!msg.guild || !command.needCompetition) return false;
